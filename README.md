...
| Command | Explanation |
|----|----|
|driver.executescript("mobile: isBiometricEnrolled")|Return True or False based on whether the device has biometric security enabled|
|driver.execute_script("mobile: enrollBiometric", {'isEnabled': True})|Enroll (or unenroll) the device in the biometric security program (via isEnabled parameter)|
|driver.execute_script("mobile: sendBiometricMatch", {'type': 'touchId', 'match': True})|Perform a successful or unsuccessful match. Type must be touchId or faceId.|
