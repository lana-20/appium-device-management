...
To use Appium to unlock an Android device screen that's protected with a password, pin, or pattern, the first thing we need to do is ensure we use two new capabilities when we start our session. The first is <code>unlockType</code>, and it tells Appium which unlock strategy to expect it should use. This should be a string, and should be the word <code>password</code>, <code>pin</code>, or <code>pattern</code>. The second capability is called <code>unlockKey</code>, and it is the particular value that Appium should use to unlock the device. In other words, if the unlock type is <code>password</code>, then the value of the <code>unlockKey</code> capability should be the actual password itself. And so on. When a session has these capabilities set, and you call <code>driver.unlock()</code>, Appium will use the strategy you've defined to try to unlock an Android screen! These capabilities have no effect on iOS.
